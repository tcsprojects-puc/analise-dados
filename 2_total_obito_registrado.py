...
obito_cartorio = gerarListaArquivo("base_caso.csv")
meses = obterMeses()
...
for i in range(len(obito_cartorio)) :
	if i != 0 :
		...
		if linha[1] == estado :
			for i in range(len(meses)) :
				if mes == i + 1 :
					totais_meses[i] += int(linha[5])
					eixo_x.append(meses[i])
					eixo_y.append(totais_meses[i])

gerarGrafico(tipo_grafico["l"], eixo_x, eixo_y, "Total de óbitos registrados por mês no Estado", "Eixo X", "Eixo Y", "Gráfico", "blue")
